// This script initializes various features on the Knowledge Retention Platform
...
// Handles adding and managing tasks, and updates the progress bar
...
// Displays insights generated by the platform
function handleInsights() {
    const insightsList = document.getElementById('insights-list');

    if (insightsList) {
        const addInsight = (insightText) => {
        ...
        // Example insights
        addInsight('Document1.pdf: Key summary generated.');
        addInsight('Document2.docx: Risk analysis complete.');
    }
}

// Initializes Gantt charts and bar charts for analytics
function initializeCharts() {
    const ctxBar = document.getElementById('chart-container')?.getContext('2d');
    const ganttCtx = document.getElementById('gantt-chart')?.getContext('2d');

    // Bar chart for document analytics
    if (ctxBar) {
        new Chart(ctxBar, {
            type: 'bar',
            data: {
...
// Handles adding new projects and updating the project list
...
// Adds hover effect for expanding/collapsing the sidebar
...
document.addEventListener("DOMContentLoaded", () => {
    console.log("Welcome to the Enhanced Knowledge Retention Platform!");
    ...
    const updateStats = () => {
        ...
        animateCounter(tasksCompletedEl, completedTasks);
        document.getElementById("reports-generated").textContent = "5"; // Mock data
    };
    ...
    document.getElementById("add-task").addEventListener("click", () => {
        ...
        if (!taskInput || !taskDeadline) return alert("Please fill in all task details.");

        const taskItem = document.createElement("li");
        taskItem.classList.add("task-item", taskPriority);
        taskItem.innerHTML = `
            ${taskInput} 
            <span class="deadline">Due: ${taskDeadline}</span> 
    ...
    document.querySelectorAll(".filter-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            document.querySelectorAll("#task-list .task-item").forEach(task => {
                task.style.display = filter === "all" || task.classList.contains(filter) ? "block" : "none";
            });
        });
    });
    ...
    document.getElementById("analyze-btn").addEventListener("click", () => {
        ...
        progress.value = 0;
        document.getElementById("analysis-result").appendChild(progress);

        [...files].forEach((file, index) => {
            setTimeout(() => {
                progress.value = index + 1;
                if (progress.value === files.length) {
                    document.getElementById("result-output").textContent = "Analysis complete!";
                    showNotification("Files analyzed successfully!", "info");
                }
            }, (index + 1) * 1000);
        });
    });
    ...
    document.getElementById("add-project").addEventListener("click", () => {
        ...
        if (!projectName || !projectDeadline || !projectDescription) {
            return alert("Please fill out all project fields.");
        }
        ...
        projectCard.querySelector(".complete-btn").addEventListener("click", () => {
            projectCard.classList.add("completed");
            showNotification(`Project "${projectName}" marked as complete!`, "success");
        });

        projectCard.querySelector(".delete-btn").addEventListener("click", () => {
            projectCard.remove();
            showNotification(`Project "${projectName}" deleted.`, "warning");
        });
    ...
    // Overdue Projects Reminder
    setInterval(() => {
        const now = new Date().toISOString().split("T")[0];
        document.querySelectorAll(".project-card").forEach(project => {
            const deadline = project.querySelector("p strong")?.textContent?.replace("Deadline:", "").trim();
            if (deadline && deadline < now) {
                project.classList.add("overdue");
                showNotification(`Project "${project.querySelector("h3").textContent}" is overdue!`, "warning");
            }
        });
